...
#define VEC_IMPL(T)                                                            \
    typedef struct {                                                           \
        ...
        size_t cap;                                                            \
                                                                               ...
                                                                               \
    void __Vec_##T##_Grow(__Vec_##T *vec)                                      \
    {                                                                          \
        if (vec->cap == vec->len) {                                            \
            vec->cap = MAX(__VEC_MIN_SIZE, vec->cap * 2);                      \
            vec->items =                                                       \
                realloc(vec->items, sizeof(vec->items[0]) * vec->cap);         \
        }                                                                      \
    }                                                                          \
                                                                               \
    void __Vec_##T##_Shrink(__Vec_##T *vec)                                    \
    {                                                                          \
        if (vec->len < vec->cap / 4) {                                         \
            ...
            vec->items =                                                       \
                realloc(vec->items, sizeof(vec->items[0]) * vec->cap);         \
                                                                               ...
                                                                               \
    size_t __Vec_##T##_Push(__Vec_##T *vec, T item)                            \
    {                                                                          \
        __Vec_##T##_Grow(vec);                                                 \
    ...
    {                                                                          \
        ...
        return result;                                                         \
                                                                               ...
                                                                               \
    T __Vec_##T##_Swap_Remove(__Vec_##T *vec, size_t index)                    \
    {                                                                          \
        ...
        return result;                                                         \
                                                                               ...
                                                                               \
    T __Vec_##T##_Remove(__Vec_##T *vec, size_t index)                         \
    {                                                                          \
        T result = vec->items[index];                                          \
    ...
    void __Vec_##T##_Free(__Vec_##T *vec)                                      \
    {                                                                          \
        if (vec->items) {                                                      \
            free(vec->items);                                                  \
        }                                                                      \
